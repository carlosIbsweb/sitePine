(function ( $ ){
	...
	var modalForm = $('#pineAddCri .modal-body form');
		...
		********************/
		...
		var id 	= ID();
		let exibirDadosCrianca = $(this).data('exibir-dados-crianca') ?? true;
		let defaultForm = document.createElement('div');
		defaultForm.classList.add('formCri')
		defaultForm.setAttribute('id',id)
		defaultForm.innerHTML = 	
				'<div class="form-group buscName">'+
					'<label for="nome-crianca" class="col-form-label">'+$(this).data('texto-label-nome')+'</label>'+
				...
				/*'<div class="form-group">'+
					...
					+'</select>'+
				'</div>'+*/
				'<span class="form-dados-crianca">'+
				'<div class="form-group">'+
					'<label for="possui-doenca" class="col-form-label">A criança possui alguma doença crônica, transtorno e/ou deficiência? (Ex.: surdez, diabetes, autismo, TDAH, baixa visão…) Especifique.</label>'+
					'<input type="hidden" class="form-control" name="criancas['+id+'][possuidoenca-label]" id="label-possuidoenca" value="A criança possui alguma doença crônica, transtorno e/ou deficiência? (Ex.: surdez, diabetes, autismo, TDAH, baixa visão…) Especifique.">'+
					'<textarea required class="form-control inputPaym" name="criancas['+id+'][possuidoenca]" id="possui-doenca" autocomplete="off"></textarea>'+
					'<div class="payBuscCri" style="display:none"></div>'+
				'</div>'+
				'<div class="form-group">'+
		...
		//Ocultando os dadso da criança pelo atributo data exibir-dados-crianca
		if(!exibirDadosCrianca){
			defaultForm.querySelector('.form-dados-crianca').remove()
		}
		...
		**/
		...
		$( document ).on('click','.cCriEdit',function(){
			...
			var $cloned = editLoadFormId.clone();


			var $originalSelects = editLoadFormId.find('select');
			$cloned.find('select').each(function(index, item) {
			     //set new select to value of old select
			     $(item).val( $originalSelects.eq(index).val() );
			});

			//Removendo o option de adicionar nova escola, para não exibir na hora de editar a criança.
			$cloned.find('.nescola').remove();
		...
		**********/
			...
			jQuery.ajax({
			  ...
			  success: function( response ) {
			  	...
			  	$.each(data.dados,function(ind,item){
			  	   ...
			  	   form.find('#possui-doenca').val(item.possuidoenca);
			  	   form.find('#possui-doenca').attr('name','criancas['+pId+'][possuidoenca]');
			  	   form.find('#label-possuidoenca').attr('name','criancas['+pId+'][possuidoenca-label]');
			  	   ...
			  	   form.find('#label-nascimento').attr('name','criancas['+pId+'][nascimento-label]');
			  	  	...
			  	  	form.find('#nome-escola').val(item.escola).change();
			  	   //form.find('#nome-escola option[value="'+item.escola+'"]')
		...
		$( document ).on('submit','#add_n_escola',function(event){
			event.preventDefault();

			$data = $(this).find('[name="escola"]').val()

			if($data.replaceAll(' ','').length < 3){
				alert('Nome não pode ser vazio ou menor do que 3 caracteres!')
				return false;
			}
			...
			jQuery.ajax({
			  url: '/components/com_s7dpayments/tmpl/add_escolas.php',
			  type: 'POST',
			  data: {nome:''+$data+''},
			  success: function( response ) {
			  	...
			  	$('#nome-escola').find('.nescola').before(`
			  		<option value="${$data}">${$data}</option>
			  	`)
			  	$('#nome-escola').val($data).change()
			  	$("#pineAddEsc").modal('hide')

			  	$form.find('[name="escola"]').val('')	
			  },
			  error: function(xhr, textStatus, errorThrown) {
			    //called when there is an error
			  }
		...
		$('#pineAddEsc').on('hide.bs.modal', function (e) {
  			$('#add_n_escola').find('[name="escola"]').val('')
  			
  			if($('#nome-escola').val().toLowerCase() == "naescola"){
  				$('#nome-escola').val('')
  			}
		})
