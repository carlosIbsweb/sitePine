...
class S7dpaymentsViewPayments extends JViewLegacy
{
    ...
    ********************/
    ...
    public static function getCriList($exEscola = false,$qualIdade = 14,$faixa = 'Crianças',$cond = '<')
    {
        ...
        foreach(self::getCatList($_SESSION['fId']) as $ks=> $cat)
        {
            foreach(self::getCatList($cat->id) as $c)
            {
                ...
                array_push($escolas,self::getCriListT($c->id,false,1,$qualIdade,$cond));
                array_push($emails,self::getCriListT($c->id,false,2,$qualIdade,$cond));
            }
        }
        ...
        /*************
         Lista json
        *************/
        echo '<div class="filterFaixaEtaria">'.$faixa.'</div>';
        ...
        foreach(self::getCatList($_SESSION['fId']) as $ks=> $cat)
        {
            ...
            foreach(self::getCatList($cat->id) as $c)
            {
                if(json_decode($c->params)->avulso != 1)
                {
                    $soma = $soma + self::getCriListT($c->id,1,false,$qualIdade,$cond);
                    self::getCriListT($c->id,'',false,$qualIdade,$cond);
                 }
            }
            echo '</tbody>';
        }
        ...
        if(!empty($soma))
        {
            echo '<p class="piT"><strong>Total:</strong> '.$soma.'</p>';
        }

    }

    public function transporteGuiamedico() {
        //Transporte -----------
       /* echo '<h3 style="text-align:center; margin-top: 30px; color:#ff9603">Transporte <i class="fa fa-bus" aria-hidden="true"></i></h3>';
        echo '<table class="criList">';
        echo  
        '<thead>
            <th width="20%">Nome</th>
        ...
        foreach(self::getCatList($_SESSION['fId']) as $ks=> $cat)
        {
            ...
            foreach(self::getCatList($cat->id) as $c)
            {
                if(json_decode($c->params)->avulso == 1)
                {
                    $somaT = $somaT + self::getCriListT($c->id,1,false,$qualIdade,$cond);
                    echo   self::getCriListT($c->id,'',false,$qualIdade,$cond);
                 }
            }
            echo '</tbody>';
        }
        ...
        if(!empty($somaT)){
            echo '<p class="piT"><strong>Total:</strong> '.$somaT.'</p>';
        }*/
        ...
        echo '<tr><td><strong>Perg.4</td><td>A criança possui alguma doença crônica, transtorno e/ou deficiência? (Ex.: surdez, diabetes, autismo, TDAH, baixa visão…) Especifique.</td></tr>';
        echo '</table>';
        echo '</div>';
    }
    ...
    *******************/
    ...
    public static function getCriListT($catid,$count = '',$exEscola = false, $qualIdade,$cond = '<')
    {
        // Initialiase variables.
        $db    = JFactory::getDbo();
        $query = $db->getQuery(true);
        ...
        foreach($result as $k=> $item)
        {
            foreach(json_decode($item->items) as $ks=> $dados)
            {
                ...
                {   
                    $idade = self::calcularIdade($cri->nascimento,date('Y-m-d'));

                    $cond = $cond == '>' ? $idade > $qualIdade : $idade < $qualIdade;
                    ...
                    //if($cond){ 
                        ...
                        if(($cri->escola == $_POST['escolaN']) || !isset($_POST['escolaN']) || empty($_POST['escolaN']) ) {
                            ...
                            $telinter = !empty(JFactory::getUser($item->userid)->telefoneinternacional) ? '<br>Inter - '. JFactory::getUser($item->userid)->telefoneinternacional : null;
                            $output .= '<tr>';
                            ...
                            $output .= '<td>'.ucfirst(mb_strtolower($cri->possuidoenca)).'</td>';
                            $output .= '<td>1 - '.self::titleCase(JFactory::getUser($item->userid)->name).$resp2.'</td>';
                            $output .= '<td>'.JFactory::getUser($item->userid)->email.'</td>';
                            $output .= '<td> 1 - '.JFactory::getUser($item->userid)->telefone.$tel2.$telinter.'</td>';
...
function calcularIdade($data_nascimento, $data_referencia = null) {
    ...
    try {
        // Converte as datas para objetos DateTime para facilitar o cálculo
    ...
    $diferenca = $data_nascimento->diff($data_referencia);
...
function faixaEtaria($idade){
    return $idade > 14 ? 'Adulto' : 'Criança';
}
