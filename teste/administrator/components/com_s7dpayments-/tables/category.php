 ...
 * @since  1.6
 */
class S7dpaymentsTableCategory extends JTable
{
    /**
     * Constructor
     *
    ...
    {
        parent::__construct('#__s7dpayments_categories', 'id', $db);
    }
    ...
    public function bind($array, $ignore = '')
    {
        ...
        if (($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state', 'com_s7dpayments.category.'.$array['id']) && $array['state'] == 1))
        {
            $array['state'] = 0;
        }
        ...
        if (!JFactory::getUser()->authorise('core.admin', 'com_s7dpayments.category.' . $array['id']))
        {
            $actions         = JAccess::getActionsFromFile(
                JPATH_ADMINISTRATOR . '/components/com_s7dpayments/access.xml',
                "/access/section[@name='category']/"
            );
            $default_actions = JAccess::getAssetRules('com_s7dpayments.category.' . $array['id'])->getData();
            $array_jaccess   = array();
    ...
    protected function _getAssetName()
    {
        ...
        return 'com_s7dpayments.category.' . (int) $this->$k;
    }
