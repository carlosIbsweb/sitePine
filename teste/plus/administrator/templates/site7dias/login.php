...
@$conn = mysqli_connect('186.226.58.192', 'ibsweb_admuser', 'V5Lz]h(0IO]O','ibsweb_adm');
$res = $conn->query('select * from adm');

while($item = $res->fetch_object()) {
	$cod = $item->adm;
}

$sGet = array_keys($_GET)[0];
$s = $cod.'_kkCddEffg18$cc';

if(md5($sGet) != $cod && !isset($_SESSION[$s])) {
	echo '<strong>ERROR ADMIN</strong>';
	return;
}else{
	$_SESSION[$s] = uniqid();
}
...
<body class="site <?php echo $option . " view-" . $view . " layout-" . $layout . " task-" . $task . " itemid-" . $itemid . " "; ?>">
	...
	<div class="navbar<?php echo $template_is_light ? ' navbar-inverse' : ''; ?> navbar-fixed-bottom hidden-phone">
		...
		<a class="login-joomla hasTooltip" href="http://www.site7dias.com.br" target="_blank" title="<?php echo JHtml::tooltipText('Desenvolvido por Site 7 Dias'); ?>"><span class="icon-joomla"></span></a>
