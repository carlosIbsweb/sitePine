          ...
          success: function (response) {
            setTimeout(function(){
              ...
              if(typeof data == 'object')
              {
                if(data.success == true)
                {
                  ...
                  if(data.redirect != false){
                    $(".aw-form-loader").fadeOut(function(){
                      $(this).removeClass('aw-form-loader').addClass('aw-form-loader-redirect')
                      $(this).html( '<div class="redirectAw" style="display:none"><div class="rediContent">'+ data.menredirect+'<div class="awLoader-15 loading"></div></div></div>').fadeIn();
                      $('.redirectAw').fadeIn();
                    });

                    if(data.redirectDownload != true){
               ...
               $('body').find('.aw-form-loader-redirect').fadeOut(300, function(){
                $(this).remove();
              })
                      },data.redirectTime)
    ...
    var masArr = $.makeArray($(opt.formId + ' input[type="tel"]'));
      ...
      $.each(masArr,function( ind, val){
        if($( this ).data('mask')){
          ...
          //Criando mascara no telefone para evitar que apague ao chamar a função da mascara
