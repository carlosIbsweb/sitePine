		...
		foreach(json_decode(json_encode($t->stages)) as $id => $r){
			foreach($r->rows as $l){
				foreach(json_decode(json_encode($t->rows)) as $rid => $rows){
					if($rid === $l){
						$classRow = $rows->config->legend;
						$classRow = explode('aw-add',$classRow);

						$awAdd = count($classRow) > 1 ? true : false;
						$classR = $awAdd ? 'aw-add' : $classRow[0];
						$awAddMen = $classRow[1];


						echo '<div class="aw-form-row row '.$classR.'">';

						echo $awAdd ? '<div class="aw-form-container">' : '';
						...
						if($awAdd)
						{
							$awid = uniqid();
							echo '
							<div class="aw-add-items" id="awadditems-'.$awid.'" data-id="'.$awid.'"></div>
							<div class="aw-btn-add"><button type="button" class="'.$classRow[0].'">'.$awAddMen.' <i class="fa fa-plus-circle" aria-hidden="true"></i></button></div>';
						}
	...
	foreach($colFields as $colField){
		foreach(json_decode(json_encode($dados)) as $fId => $field){
			if($fId === $colField){
				if(isset($_GET['awEdit']))
				{
					awRenderEdit::render($field,$wColumn,$captcha,$moduleId,$dbName);
				}else{
