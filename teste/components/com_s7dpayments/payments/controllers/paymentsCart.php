	...
	*********/
	...
	{	
       ...
       foreach(self::getCat($_GET['cat']) as $cat):
		...
		if(!$diariaD){
			echo '<div class="alert alert-danger paymentsAlert" role="alert"> <strong>Erro!</strong>Máximo de diárias permitido é 3.</div>';
			return false;
		}

		if(isset($list['semana']) && $list['semana'] == 'diaria' )
		{
	...
	public static function setDiscount($discount,$cartid,$userId,$discountId)
	{
		...
		if(self::getDiscontUniq($discountId)){
			echo '<div class="alert alert-danger cf-alert" role="alert">Voucher Encerrado!</div>';
			return false;
		}
