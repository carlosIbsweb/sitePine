...
/*
  data-combo = |campo|:['valor>label'] or data-combo1 or data-combo2
  data-name = 'nome,nome2,nome3'
*/

$( document ).on('change','[data-combo]',function(e){
  e.preventDefault();
  
  let el = $(this)
  
  let cb = $(this).val()
  let cbn = $(this).data('name').split(',')

  cbn.forEach(function(v,k){
    
    let pcombo = k == 0 ? '' : k;
    
    let cbs = '{'+el.data('combo'+pcombo).replaceAll('|','"').replaceAll("'",'"')+'}'
    
    let cbso = JSON.parse(cbs)[cb]

    if(!(cbso instanceof Array)){
      $('select[name="'+v+'"]').html('<option value="">-</option>')
      return;
    } 
    ...
    {
      let gval = opt.split('>')
      let [optval,optlabel] = gval.length > 1 ? gval : [opt,opt]
